...
- [4. Volatility](#4-volatility)
  ...
  - [4.5 Stochastic-Local Volatility](#45-stochastic-local-volatility)
...
- [9. Interest-Rate Derivatives](#9-interest-rate-derivatives)
  - [9.1 Change of Numeraire and the Forward Measure](#91-change-of-numeraire-and-the-forward-measure)
  - [9.2 Caps, Floors, and the Caplet Formula](#92-caps-floors-and-the-caplet-formula)
  - [9.3 Swaptions and the Annuity Measure](#93-swaptions-and-the-annuity-measure)
  - [9.4 Bermudan Swaptions and Early Exercise](#94-bermudan-swaptions-and-early-exercise)
  - [9.5 CMS and Convexity Adjustments](#95-cms-and-convexity-adjustments)
  - [9.6 Model Choice for Rate Derivatives](#96-model-choice-for-rate-derivatives)
...
The risk-neutral measure of Section [1.1](#11-no-arbitrage-martingales-and-risk-neutral-pricing) is the special case $N_t = B_t = \exp\!\int_0^t r_s\,ds$ (the money-market account). Rate derivatives are far easier under a different choice.
...
The simply-compounded **forward rate** seen at $t$ for accrual period $[T_{i-1}, T_i]$ with year fraction $\tau_i$ is, from Section [3.2](#32-single-curve-vs-multi-curve-framework),
...
and it is a martingale under the $\mathbb{Q}^{T_i}$-forward measure that uses the **discount** (OIS) bond $P(\cdot, T_i)$ as numéraire — the split of the two curves is exactly the multi-curve content of Section [3.7](#37-no-arbitrage-relations-across-curves).

A **caplet** pays $\tau_i\,(F_i(T_{i-1}) - K)^+$ at $T_i$. Under $\mathbb{Q}^{T_i}$ with $F_i$ a martingale, the price is a Black-76 (Section [2.2](#22-black-76-futures-forwards)) expression:
...
or, for **normal** (Bachelier, Section [2.3](#23-bachelier-normal-model)) quoting — the post-2008 market standard for rates because forwards can be negative,
...
- **Longstaff-Schwartz (LSM)** on simulated LMM paths: regress the continuation value on curve state variables at each exercise date, then exercise when immediate value exceeds the regressed continuation. Implemented in `valax/pricing/mc/bermudan.py` on the LMM of Section [2.6](#26-libor-market-model-lmm-bgm).
- **Backward induction on the Hull-White trinomial tree** (Section [2.8](#28-hull-white-one-factor-short-rate-model)): the single short-rate state makes the exercise decision a per-node $\max(\text{continuation}, \text{exercise})$ — the same machinery already used for callable bonds.
- **Short-rate PDE** (roadmap PR-3): the exercise projection seam of the finite-difference solver (Section [5.2](#52-pde-finite-differences)) carries over directly. Not yet built.
...
|------|-------|-----|---------|
| Callable/puttable bonds, exact curve fit | **Hull-White 1F** | Single state, affine ZCB, exact-fit; tree/PDE for exercise | [2.8](#28-hull-white-one-factor-short-rate-model) |
| European swaption *level* (fast calibration) | **Hull-White** via Jamshidian | Decompose into a portfolio of ZCB options | [2.8](#28-hull-white-one-factor-short-rate-model) |
| Full forward-curve dynamics, Bermudans | **LMM / BGM** | Models each forward directly; PCA-reduced factors | [2.6](#26-libor-market-model-lmm-bgm) |
| Swaption/cap **smile** | **SABR** per expiry | Fits the strike dimension the one-factor models cannot | [2.5](#25-sabr) |
